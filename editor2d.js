...
export class Editor2D {
  ...
  _cancelSingle() {
    ...
    this.dragging = null; this.rotating = null;
    if (this.panning) this._endPan();
  }
  ...
  _pdown(sx, sy) {
    ...
    if (tool === 'select') {
      // Check handles on currently selected item
      if (this.state.selectedId) {
        ...
        if (!isRoom) {
          ...
          if (room) {
            ...
            const dx = s.x + room.width * this.zoom, dy = s.y;
            if (Math.hypot(sx - dx, sy - dy) < 14) {
              this.state.rooms = this.state.rooms.filter(r => r.id !== room.id);
              this.state.selectedId = null;
              this.cb.onRemove(); this.cb.onUpdate(); this.render(); return;
            }
          }
      ...
      // Hit test furniture first, then rooms
  ...
  _pmove(sx, sy) {
    ...
    if (this.state.tool === 'room' && this.roomDrag) {
      ...
      const w   = Math.abs(sn.x - this.roomDrag.x);
      const h   = Math.abs(sn.y - this.roomDrag.y);
      this._roomPrev = { x, y, width: w, height: h };
      this.render(); return;
    }
  ...
  _keydown(e) {
    ...
    } else if (e.key === 'r') { this.cb.onToolChange('room');   }
  ...
  _delHS(it) {
    const lx = it.width/2, ly = -(it.depth/2);
    const c = Math.cos(it.rotation), s = Math.sin(it.rotation);
    return this.w2s(it.x + lx*c - ly*s, it.y + lx*s + ly*c);
  }
  ...
  _drawRooms(ctx) {
    for (const room of this.state.rooms) {
      ...
      // Border
      ctx.strokeStyle = sel ? '#ffffff' : room.color;
      ...
      if (sel) {
        const dx = s.x + rw, dy = s.y;
        ctx.fillStyle = '#E94560'; ctx.strokeStyle = '#fff'; ctx.lineWidth = 1.5;
        ctx.beginPath(); ctx.arc(dx, dy, 9, 0, 6.28); ctx.fill(); ctx.stroke();
  ...
  _drawRoomPreview(ctx) {
    ...
    const r  = this._roomPrev;
    const s  = this.w2s(r.x, r.y);
    const rw = r.width  * this.zoom;
    const rh = r.height * this.zoom;

    ctx.globalAlpha = 0.12;
    ctx.fillStyle = '#63b4ff';
    ctx.fillRect(s.x, s.y, rw, rh);
    ctx.globalAlpha = 1;

    ctx.strokeStyle = '#5B9AD5'; ctx.lineWidth = 2;
    ctx.setLineDash([7, 4]); ctx.strokeRect(s.x, s.y, rw, rh); ctx.setLineDash([]);
  ...
  _drawWalls(ctx) {
    ...
    if (this.wallStart) {
      ...
      if (this.state.tool === 'wall' && this.mouseW) {
        ...
        const s1  = ss, s2 = this.w2s(end.x, end.y);
  ...
  _drawFurniture(ctx) {
    ...
    for (const it of this.state.furniture) {
      ...
      const pw  = it.width * this.zoom;
      const ph  = it.depth * this.zoom;
      ...
      if (sel) {
        ...
        const dh = this._delHS(it);
        ctx.fillStyle = '#E94560'; ctx.strokeStyle = '#fff'; ctx.lineWidth = 1.5;
        ctx.beginPath(); ctx.arc(dh.x, dh.y, 9, 0, 6.28); ctx.fill(); ctx.stroke();
